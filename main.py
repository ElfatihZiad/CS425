...
@app.post("/", response_model=RegistrationCreate, status_code=201)
...
@app.get("//{workshop_id}", response_model=RegistrationResponse)
...
def seed_data():
    ...
    try:
        if db.query(Workshop).count() == 0:
            db.add_all(
                [
                    Workshop(title="Spring Boot Essentials", description="REST APIS", date=date(2026, 9, 2), time=time(12,12,12), deadline = datetime(2026, 10, 10, 2,0,0)),
                    Workshop(title="Software Engineering", description="...", date=date(2026, 9, 2), time=time(12,12,12), deadline = datetime(2026, 10, 10, 2,0,0)),
                    Workshop(title="Machine Learning", description="...", date=date(2026, 9, 2), time=time(12,12,12), deadline = datetime(2026, 10, 10, 2,0,0)),
